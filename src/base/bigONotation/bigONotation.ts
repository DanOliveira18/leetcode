...
// é O(n)
export const printAllAndInverted = (items: Array<any>) => {
  //O(n)
  items.forEach((item) => {
    console.log(item); //O(1)
  });
...
export const findElement = (matrix: Array<Array<number>>, target: number) => {
...
export const combineNames = (arr: Array<string>) => {
  // O(n^2)
